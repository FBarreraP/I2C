...
void TestConnection(void){
    clearLCD();
    WriteLCD(text1,0x80,sizeof(text1));
    ReadI2C(0xA0,0x75,1,&data);
    if (data != 0x68){
        WriteLCD(text2,0xC0,sizeof(text2));
        sprintf(text,"Eu sou: %x",data);
        WriteLCD(text,0xC0,sizeof(text));
        LATBbits.LATB6 = 1;

    }else{
        WriteLCD(text3,0xC0,sizeof(text3));
        LATBbits.LATB7 = 1;
    }
    _delay((unsigned long)((10*5)*(20000000/4000.0)));
}

void settingsIMU(void){
    clearLCD();
    WriteLCD(text4,0x80,sizeof(text4));
    WriteI2C(0xA0,0x1B,&a,1);
    WriteI2C(0xA0,0x1C,&a,1);
}

void start(void){
    ReadI2C(0xA0,0x3B,14,GyrAccel);
